...
equation
  ...
  phi_dotdot = (k_ride_f*t*(z_tyre_fl-z) - k_ride_f*t*(z_tyre_fr-z) + k_ride_r*t*(z_tyre_rl-z) - k_ride_r*t*(z_tyre_rr-z) + cf*t*(z_tyre_fl_dot-z_dot) - cf*t*(z_tyre_fr_dot-z_dot) + k_ride_r*t*(z_tyre_rl_dot-z_dot) - k_ride_r*t*(z_tyre_rr_dot-z_dot) - (k_ride_f+k_ride_r)*t^2*phi - (k_ride_f+k_ride_r)*t^2*phi - (cf+cr)*t^2*phi_dot - (cf+cr)*t^2*phi_dot)/Ixx;
  ...
  if time - (wheelbase/velocity)/2 < 0 then
    z_road_fr = 0;
  elseif time < time_from_start + (wheelbase/velocity)/2 then
    z_road_fr = step_height/2*(1-cos(pi*(time-(wheelbase/velocity)/2)/time_from_start));
  else
  ...
  if time - (wheelbase/velocity)*3/2 < 0 then
    z_road_rr = 0;
  elseif time < time_from_start + (wheelbase/velocity)*3/2 then
    z_road_rr = step_height/2*(1-cos(pi*(time-(wheelbase/velocity)*3/2)/time_from_start));
  else
